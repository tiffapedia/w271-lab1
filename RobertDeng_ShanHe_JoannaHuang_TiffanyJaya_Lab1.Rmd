...
With a p-value of 0.0320, there is sufficient evidence to conclude that outside temperature at the time of the launch does affect the probability of at least one O-ring failure.
...
5. The outcome variable does not separate the predictor variable completely. We can validate this does not happen because R will give a warning message when it does. In addition, the standard error for the Temp coefficient is not extremely large, and looking at the relationship between the outcome variable and the explanatory variable, we can see that there are failures still occuring at warmer temperature as well as cooler ones. 
6. Observations are independent of one another. Two assumptions need to be tested for this model:
  a. 
  b. 

Thanks to the assumption check we have made, we can continue predicting the probability of at least one O.ring failure at the temperature when they launch the Challenger, 31°F.  Based on our logistic regression model bm2, the probability of at least O-rings failing is approximately $\hat{\pi_i} \approx 0.99.$ If they have waited until 72 °F as suggested, it would have reduced the probability of at least one O-ring failing to approximately 0.16. 
...
For every 1 degree Fahrenheit decrease in temperature, the odds of the O-rings failing for a given launch increases by 26% or a factor of 1.26. 
...
```{r}
exp(bm2.b1  * (rec.temp - temp.launch))
```
