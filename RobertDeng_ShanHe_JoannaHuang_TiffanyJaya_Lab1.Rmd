...
```{r}
d <- read.table(file = "./dataset/challenger.csv", header=TRUE, sep=",")
summary(d)
```
```{r}
...
```{r}
plot(x = d$Temp, y =d$O.ring, ylab='Number of O-ring failures', xlab='Temp(F)',
...
```{r}
plot(x = d$Pressure, y =d$O.ring, ylab='Number of O-ring failures', xlab='Pressure(psi)', main = 'Combustion Pressure Versus Number of O-Ring Failures', xlim=c(50,200), ylim=c(0,3))
```
...
```{r}
d.glm.H0 <- glm(Fail ~ Temp, family = binomial, data=d)

anova(d.glm.H0, d.glm, test='Chisq')
```
```{r}
library(car)
