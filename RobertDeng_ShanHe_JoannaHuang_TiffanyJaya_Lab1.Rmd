...
<span style='color:red'>**(Answer 5D and 5E)**</span> This model meets the necessary assumptions and we can continue predicting the probability of at least one O-ring failure with greater certainty. Based on our logistic regression model bm2, the probability of at least one O-ring failing when they launch the Challenger at 31°F is approximately $\hat{\pi_i} \approx 0.99$ with 90% confidence interval of (0.99, 1.00) found using parametric bootstrap. If they have waited until 72 °F as suggested, it would have reduced the probability of at least one O-ring failure to approximately 0.16 with 90% confidence interval of (0.3968449, 0.9763451) found using the same parametric bootstrap. 
...
**(Answer 3A)** To interpret our model, as we can see below, for every 1 degree Fahrenheit decrease in temperature, the odds of the O-rings failing for a given launch increases by 26% with the true population effect between 1.02 and 1.56. 
...
<span style='color:red'>**(Answer 5B and 5C)**</span>  With a selected model bm2, we can then investigate how the estimated probability changes over different tempratures. 

```{r fig.show='hold', fig.align='center', out.width='49%'}
# Plot estimated pi
curve(predict(object=bm2, data.frame(Temp=x), type="response"), xlim = c(31,81),
