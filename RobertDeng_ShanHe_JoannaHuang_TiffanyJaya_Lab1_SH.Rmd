...
```{r}
setwd("/Users/shanhe/Desktop/W271/Lab/w271_lab1/dataset")
...
**feel like we can elaborate more here**
...
**felt like geom_point was good enough since we don't have too many points that overlap**

```{r}
plot(x = jitter(challenger$Pressure), y =challenger$O.ring, ylab='Number of O-ring failures', xlab='Pressure(psi)', main = 'Combustion Pressure Versus Number of O-Ring Failures', xlim=c(50,200), ylim=c(0,3))
```
**Looking at this plot, there doesn't appear to be any particular relationship between O-ring failures and pressure.**\ 

**used jitter here since we have many overlapped points**
...
```{r}
cor(challenger[, c("O.ring", "Temp", "Pressure")])
# install.packages("corrplot")
library("corrplot")
library(corrplot)
corrplot(cor(challenger[, c("O.ring", "Temp", "Pressure")]), method="circle")
#I don't think Jeff likes the idea of adding unneccessary library but this can be helpful for us to visualize. can remove in the final version
...
```{r}
lp.31 <- predict(object = challenger.glm.temp, newdata= data.frame(Temp=31), type = 'link', se = TRUE)
...
coefficients(challenger.glm.temp)
...
replicate(10000, bootstrap_coef(challenger$Temp, challenger.glm.temp, 31), simplify = FALSE)

replicate(10000, bootstrap_coef(challenger$Temp, challenger.glm.temp, 72), simplify = FALSE)
```
...
```{r}
challenger.glm.temp
challenger.glm.temp.quad <- glm(formula = Fail ~ Temp + I(Temp^2), family = binomial(link=logit), data = challenger) 
summary(challenger.glm.temp.quad)
anova(challenger.glm.temp, challenger.glm.temp.quad, test='Chisq')
```
