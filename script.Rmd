...
```{r}
ggplot(d, aes(x=Temp, y=O.ring)) + geom_point() + 
...
```{r}
#Anova Test of different means for O-Ring Temps and Pressures: 
temp_aov <- aov(Temp ~ O.ring, data = d)
summary(temp_aov)
pres_aov <- aov(Pressure ~ O.ring, data = d)
summary(pres_aov)
#Magnitude of difference in means for Temp ~ O.ring
