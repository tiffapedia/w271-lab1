...
xlim.min <- 30; xlim.max <- 80; temp.launch <- 31; rec.temp <- 72
```
...
```{r}
get.pi.hats <- function(given.temps, given.model, given.formula, sample.size, total.O.rings, which.temp){
  ...
  estimated.model <- suppressWarnings(glm(formula=given.formula, family=binomial(link='logit'), data=newdata))
  
  # predict probability of O-rings failure at 
  estimated.pi.hat <- predict(object=estimated.model, newdata=data.frame(Temp=which.temp), type='response')
  
  return(estimated.pi.hat)
}

get.ci.w.parametric.bootstrap <- function(given.temps, given.model, given.formula, sample.size, total.O.rings, which.temp, num.datasets, percent.ci) {
  estimated.pi.hats <- replicate(num.datasets, get.pi.hats(given.temps, given.model, given.formula, sample.size, total.O.rings, which.temp))
  return(quantile(estimated.pi.hats, c(1-percent.ci, percent.ci)))
}
...
get.ci.w.parametric.bootstrap(given.temps=d$Temp, given.model, given.formula, 
                              sample.size, total.O.rings, which.temp=temp.launch,
                              num.datasets=1000, percent.ci=.9)
