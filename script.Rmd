...
bootstrap <- function(sample.size, total.O.rings, pi.hat, pred.temp){
  # simulate number of O-rings failure
  data$O.ring <- rbinom(n=sample.size, size=total.O.rings, pi.hat)
  data$Number <- replicate(sample.size, total.O.rings)
  
  # estimated model 
  estimated.m <- glm(formula=O.ring/Number ~ Temp, weights=Number, family=binomial(link=logit), data=data)
  
  # make predictions
  estimated.pi.hat <- predict(object=estimated.m, newdata=data.frame(Temp=temp.launch), type='response')
...
# 3. pi.hat from the model used
m <- glm(formula=O.ring/Number ~ Temp, weights=Number, family=binomial, data=d)
pi.hat <- predict(object=m, newdata=data.frame(Temp=d$Temp), type='response')

# 4. provide predicted temperature
pred.temp <- 31

# 5. 
estimated.pi.hats <- replicate(10000, bootstrap(sample.size, total.O.rings, pi.hat, pred.temp))
