...
abline(reg=lm1, col = "red", lwd = 2)
lm1.predict <- predict(lm1, data.frame(Temp=x), interval="predict")
lines(x, lm1.predict[,2], lty=2, col="red")
lines(x, lm1.predict[,3], lty=2, col="red")
```
...
```{r}
n <- sum(d$Number)
logm1 <- glm(formula = O.ring/Number ~ Temp + Pressure, 
            weights = Number, 
            family = binomial, 
            data = d)
summary(logm1)
...
```{r}
predict(bm1, data.frame(Temp = 31), type='response')
```
...
```{r}
predict(bm1, data.frame(Temp = 31), type='response')^5
```
